class SiteHelper:
    ...
    def parse_housetype_to_url(self, init_data):
    ...
    def parse_room_to_url(self, init_data):
        if len(init_data) == 1:
            return init_data
    ...
    def parse_adtype_to_url(self, init_data):
        if init_data == "Снимать":
            return "sdam"
        if init_data == "Купить":
            return "prodam"

    def parse_loc_to_url(self, init_data):
        if init_data == "Пермь":
            return "perm"
    ...
    def get_path(self, users, id, site):
        location = self.parse_loc_to_url(users[f'{id}']["city"])
        ad_type = self.parse_adtype_to_url(users[f'{id}']["ad_type"])
        count_room = self.parse_room_to_url(users[f'{id}']["count_room"])
        ...
        if ad_type == "sdam":
            if isinstance(count_room, list):
                for i in count_room:
                    paths.append(
                        f"{site}/{location}/{ad_type}/{i}-komnatnye/")
            else:
                return f"{site}/{location}/{ad_type}/{count_room}-komnatnye/"
        else:
            house_type = self.parse_housetype_to_url(users[f'{id}']["house_type"])
            if isinstance(count_room, list):
                for i in count_room:
                    paths.append(
                        f"{site}/{location}/{ad_type}{house_type}/{i}-komnatnye/")
            else:
                return f"{site}/{location}/{ad_type}{house_type}/{count_room}-komnatnye/"
        return paths
